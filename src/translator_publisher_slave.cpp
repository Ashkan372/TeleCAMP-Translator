...
public:
  TranslatorPublisher(int jrp, int frp) : Joint_recv_port(jrp), Force_recv_port(frp)
  {
   ...
   pub_joints = n_.advertise<sensor_msgs::JointState>("/master/joint_states", 1000);
   pub_force = n_.advertise<geometry_msgs::Wrench>("/master/wrench_r", 1000);
  ...
  void netRecvJoint()
  {
      ...
      // receive data
      ROS_INFO("receiving master joints from outer network and publishing them in ROS:");
  ...
  void netRecvForce()
  {
      ...
      // receive
      ROS_INFO("receiving master force from outer network and publishing them in ROS:");
