...
public:
  TranslatorSubscriber(int jsp, int fsp, long dst_ip) : Joint_send_port(jsp), Force_send_port(fsp), Dest_ip(dst_ip)
  {
   ...
   sub_joints = n_.subscribe("/master/joint_states", 1000, &TranslatorSubscriber::jointCallback, this);
   sub_force = n_.subscribe("/master/wrench_r", 1000, &TranslatorSubscriber::forceCallback, this);
  ...
  void jointCallback(const sensor_msgs::JointState& msg)
  {
      ...
      ROS_INFO("sending master joints to outer network:");
  ...
  void forceCallback(const geometry_msgs::Wrench& msg)
  {
      ...
      ROS_INFO("sending master force to outer network:");
